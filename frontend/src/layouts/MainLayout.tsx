...
import { Dna, Calculator, AlignJustify, Search, Home, LogOut, User, Folder, ShieldCheck, Sun, Moon, TestTube, Crosshair } from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
      ...
      <aside className="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col transition-colors duration-200">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-slate-800 dark:text-white">BioDjango</span>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
              <span className="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Advanced Tools</span>
              <div className="mt-2 space-y-1">
                  <SidebarItem to="/primer-design" icon={TestTube} label="Primer Design" />
                  <SidebarItem to="/antibody-annotation" icon={ShieldCheck} label="Antibody Annotation" />
              </div>
          </div>
