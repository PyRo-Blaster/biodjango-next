...
import { Dna, ArrowRight, Settings } from 'lucide-react';
...
export const PrimerDesign = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || err.response?.data?.sequence?.[0] || 'Failed to design primers');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="lg:col-span-1 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-fit">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <textarea
                                value={sequence}
                                onChange={(e) => setSequence(e.target.value.toUpperCase().replace(/[^ATCGN]/g, ''))}
                            ...
                            {result.primers.map((pair, idx) => (
                                <div key={idx} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    <div className="bg-slate-50 dark:bg-slate-800/50 px-4 py-2 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                                        <span className="font-semibold text-slate-700 dark:text-slate-300">Pair #{pair.rank}</span>
                                        <span className="text-xs text-slate-500 dark:text-slate-400">Product Size: {pair.product_size} bp</span>
                                    </div>
...
const PrimerCard = ({ type, data, color }: { type: string, data: Primer, color: 'blue' | 'green' }) => {
    ...
    return (
        <div className={clsx("p-3 rounded-lg border", colorClasses)}>
            <div className="flex justify-between items-center mb-2">
                <span className="font-bold text-xs uppercase tracking-wider">{type} Primer</span>
            ...
            <div className="flex gap-4 text-xs opacity-80">
                ...
                <span>GC: {typeof data.gc_percent === 'number' ? data.gc_percent.toFixed(1) : data.gc_percent}%</span>
            </div>
        </div>
    );
};
