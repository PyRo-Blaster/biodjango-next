...
export const ProjectDetail = () => {
    ...
    const fetchDetails = async () => {
        try {
            ...
            const seqRes = await axios.get(`/api/projects/sequences/?project_id=${id}`);
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                    {sequences.length === 0 ? (
                        <div className="p-8 text-center text-slate-500">No sequences found in this project.</div>
                    ) : (
                        sequences.map((seq) => (
                            <div key={seq.id} className="p-4 hover:bg-slate-50 transition-colors group">
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-2">
                                        <FileText className="w-4 h-4 text-slate-400" />
                                        <span className="font-mono font-medium text-primary-700">{seq.name}</span>
                                    </div>
